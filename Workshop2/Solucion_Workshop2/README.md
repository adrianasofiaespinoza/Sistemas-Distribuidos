...
```bash
python Parte1_RMI_MatrixManager/server_matrix.py 0.0.0.0 12000
```

**Paso 2: Iniciar Cliente RMI (Modo Interactivo o Quick Demo)**
```bash
# Modo interactivo CLI:
python Parte1_RMI_MatrixManager/client_matrix.py localhost 12000

# O ejecute directamente la demostración automática:
python Parte1_RMI_MatrixManager/client_matrix.py localhost 12000 --demo
```
...
python Parte2_Publisher_Subscriber/pub_weather.py 15001

# Terminal 2: Publisher de Finanzas (Puerto 15002)
python Parte2_Publisher_Subscriber/pub_finance.py 15002

# Terminal 3: Publisher de Deportes (Puerto 15003)
python Parte2_Publisher_Subscriber/pub_sports.py 15003

# Terminal 4: Suscriptor suscrito a Clima y Finanzas
python Parte2_Publisher_Subscriber/subscriber_multi.py --name Client-1 --endpoints tcp://localhost:15001 tcp://localhost:15002 --topics WEATHER FINANCE

# Terminal 5: Suscriptor suscrito a TODOS los temas
python Parte2_Publisher_Subscriber/subscriber_multi.py --name Client-All --endpoints tcp://localhost:15001 tcp://localhost:15002 tcp://localhost:15003 --topics ALL
```

**Opción B: Usar Publisher Genérico**
```bash
python Parte2_Publisher_Subscriber/publisher_service.py --service TRAFFIC --port 15004 --interval 1.5
```
...
```bash
python Parte3_Pipeline_Broker/broker.py --frontend-port 13001 --backend-port 13002
```

**Paso 2: Iniciar 2 o más Trabajadores (Workers)**
```bash
# Terminal 2 (Worker 1):
python Parte3_Pipeline_Broker/worker_pipeline.py --id Worker-1 --broker-port 13002

# Terminal 3 (Worker 2):
python Parte3_Pipeline_Broker/worker_pipeline.py --id Worker-2 --broker-port 13002
```

**Paso 3: Iniciar 1 o más Fuentes de Tareas (Sources)**
```bash
# Terminal 4 (Source 1 - Enviar 15 tareas):
python Parte3_Pipeline_Broker/source_pipeline.py --id Source-1 --broker-port 13001 --tasks 15 --delay 0.3

# Terminal 5 (Source 2 - Enviar 10 tareas):
python Parte3_Pipeline_Broker/source_pipeline.py --id Source-2 --broker-port 13001 --tasks 10 --delay 0.5
```
